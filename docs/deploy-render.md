# Deploy the Cosign Core on Render

One-click-ish deploy of the hosted free-tier Core (the playground `cosign-mcp` + the dashboard point
at). Uses `render.yaml` (Blueprint / IaC).

## Deploy

1. Push is already on GitHub. In Render: **New → Blueprint → connect `simoncrean/cosign`**.
2. Render reads `render.yaml` and provisions:
   - **`cosign-core`** — an always-on web service (`pnpm --filter @cosign/api start`), health-checked
     at `/health`, auto-deploying on push to `main`. TLS + a `*.onrender.com` URL are automatic.
   - **`cosign-ledger`** — a managed Postgres; `DATABASE_URL` is injected into the web service, so the
...
- Point the MCP server at it: `COSIGN_URL=https://cosign-core-xxxx.onrender.com` (remote mode).
...
- **`COSIGN_DEMO_TRAFFIC`:** `on` deploys the demo Core (mock fleet + synthetic spends, good for a
